...
**Пропавшая сессия:** raw-tool с ``session_id``, которой нет на сервере (не открыта, закрыта,
вытеснена по LRU, истекла по TTL или сервер перезапустился) → ``SESSION_NOT_FOUND``. Сессия не
пересоздаётся: её ``scenario_id`` потерян, и прогон молча перешёл бы на корневой датасет. Клиент
заново вызывает ``open_session`` с тем же ``scenario_id``. Неявно создаётся только ``"default"``.
